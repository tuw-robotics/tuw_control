...
template <class TNumType, class TMapDataType> using LatticeTypeStateSimBeginEndSim    = LatticeTypeStateSimBeginEnd   <TNumType,TMapDataType>;
template <class TNumType, class TMapDataType> using LatticeTypeStateSimEqDtSim        = LatticeTypeStateSimEqDt       <TNumType,TMapDataType>;
template <class TNumType, class TMapDataType> using LatticeTypeStateSimEqDsSim        = LatticeTypeStateSimEqDs       <TNumType,TMapDataType>;
template <class TNumType, class TMapDataType> using LatticeTypeStateSimCtrlPtKnotsSim = LatticeTypeStateSimCtrlPtKnots<TNumType,TMapDataType>;

using                          NumType     = double;
using                          MapDataType = double;//dummy
template<class TNumType> using DiscrType   = heun_abc<TNumType>;
// using StateDiffDriveType = StateDiffDriveVW;
// template<template<class> class TDiscretizationType> using StateSimDiffDriveType = StateSimDiffDriveVW<TDiscretizationType>;
...
// //             for ( size_t j = 0; j < trajSim->simLatticeI(i).statePtr->stateSize(); ++j ) { cout<<trajSim->simLatticeI(i).statePtr->data()(j)<<", "; }cout<<endl;
//         } cout<<endl;
        for ( size_t i = 1; i < trajSim->simLatticeSize(); ++i ) { if( !(trajSim->simLatticeI(i-1).arc < trajSim->simLatticeI(i).arc) ) { consistent = false; break; } }
        return consistent;
    }
    size_t sumAllPartStates (TrajectorySimulatorSPtr _tsp) {
        size_t nrPartStates = 0; 
	for_each_tuple(_tsp->partialLattices_, [&nrPartStates](auto& partLatticeI){ nrPartStates += partLatticeI.lattice.size(); });
        return nrPartStates;
    }
//     void checkSimLatticeStatesEqual ( vector <TrajectorySimulatorType::LatticePointType >& _latticeRef/*, vector <TrajectorySimulatorType::LatticePointType >& _latticeTest*/ ) {
...
//         for ( size_t i = 0; i < trajSim->simLatticeSize(); ++i ) { 
//             for ( int j = 0; j < trajSim->simLatticeI(i).statePtr->data().size(); ++j ) { EXPECT_DOUBLE_EQ( _latticeRef[i].statePtr->data()(j), trajSim->simLatticeI(i).statePtr->data()(j) ); }
//         }
//     }
//     void checkPartLatticeStatesEqual ( TrajectorySimulatorType::LatticeVecSPtrVec& _partLatRef, TrajectorySimulatorType::LatticeVecSPtrVec& _partLatTest ) {
...
TEST_F ( TrajectorySimulatorTest, SimPrecalcFrom0Dt ) {
    ...
    size_t statesBeginEnd  = 2, statesDt, statesDs, statesLattice0; double sumStates = 0;
    
    latticeDt.setDt(0.1); trajSim->simulateTrajectory();     
    statesDt = expectedDtStates( funcs->funcsArcEnd(), latticeDt.dt() ); statesDs = 0; statesLattice0 = 0; sumStates = statesBeginEnd + statesDt + statesDs + statesLattice0;
    EXPECT_EQ( sumStates, sumAllPartStates (trajSim) ); 
    EXPECT_EQ( sumStates, trajSim->simLatticeSize() );
    checkSimLatticeConsistency();
    
    latticeDt.setDt(0.12); trajSim->simulateTrajectory();
    statesDt = expectedDtStates( funcs->funcsArcEnd(), latticeDt.dt() ); statesDs = 0; statesLattice0 = 0; sumStates = statesBeginEnd + statesDt + statesDs + statesLattice0;
    EXPECT_EQ( sumStates, sumAllPartStates (trajSim) ); 
    EXPECT_EQ( sumStates/*statesSimExp*/, trajSim->simLatticeSize() );
    checkSimLatticeConsistency();
    
    latticeDt.setDt(0.25); trajSim->simulateTrajectory();
    statesDt = expectedDtStates( funcs->funcsArcEnd(), latticeDt.dt() ); statesDs = 0; statesLattice0 = 0; sumStates = statesBeginEnd + statesDt + statesDs + statesLattice0;
    EXPECT_EQ( sumStates, sumAllPartStates (trajSim) ); 
    EXPECT_EQ( sumStates, trajSim->simLatticeSize() );
    checkSimLatticeConsistency();
...
TEST_F ( TrajectorySimulatorTest, SimPrecalcFrom0DtDsCtrlPt ) {
    ...
    latticeDt.setDt(0.1); latticeDsVal = 0.1; trajSim->simulateTrajectory();     
    statesDt = expectedDtStates( funcs->funcsArcEnd(), latticeDt.dt() ); statesDs = 10+1; statesLattice0 = funcs->funcCtrlPtSize(0); sumStates = statesBeginEnd + statesDt + statesDs + statesLattice0;
    EXPECT_EQ( sumStates, sumAllPartStates (trajSim) ); 
//     for(size_t i = 0; i < trajSim->simLattice().size(); i++){ cout<<trajSim->simLattice()[i].arc<<", "; }cout<<endl;
    ...
    latticeDt.setDt(0.1); latticeDsVal = 0.12; trajSim->simulateTrajectory();     
    statesDt = expectedDtStates( funcs->funcsArcEnd(), latticeDt.dt() ); statesDs =  9+1; statesLattice0 = funcs->funcCtrlPtSize(0); sumStates = statesBeginEnd + statesDt + statesDs + statesLattice0;
    EXPECT_EQ( sumStates, sumAllPartStates (trajSim) ); 
    EXPECT_EQ( sumStates, trajSim->simLatticeSize() );//ds in same locations as dt
    checkSimLatticeConsistency();
    
    latticeDt.setDt(0.1); latticeDsVal = 0.01; trajSim->simulateTrajectory();     
    statesDt = expectedDtStates( funcs->funcsArcEnd(), latticeDt.dt() ); statesDs =100+1; statesLattice0 = funcs->funcCtrlPtSize(0); sumStates = statesBeginEnd + statesDt + statesDs + statesLattice0;
    EXPECT_EQ( sumStates, sumAllPartStates (trajSim) ); 
