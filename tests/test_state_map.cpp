...
TEST(TestStateMap, CopyAssignmentOperatorArray) {
    ...
    for ( int i = 0; i < nState0.data().size(); ++i ) { nState0.data()(i) = i+1; }
    nState1 = nState0;
    for ( int i = 0; i < nState1.data().size(); ++i ) { nState1.data()(i) = 0; }
    for ( int i = 0; i < nState0.data().size(); ++i ) { EXPECT_EQ(i+1,nState0.data()(i)); }
    
    for ( int i = 0; i < nState0.data().size(); ++i ) { nState0.data()(i) = i+1; }
    nState1.sub(0) = nState0.sub(0);
    for ( int i = 0; i < nState1.sub(0).data().size(); ++i ) { nState1.sub(0).data()(i) = 0; }
    for ( int i = 0; i < nState0.sub(0).data().size(); ++i ) { EXPECT_EQ(i+1,nState0.sub(0).data()(i)); }
    
    try                                   { nState1 = NestedArrayLvl1(nState0); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Copy-constructor not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    
    try                                   { nState1.sub(0) = NestedArrayLvl0(nState0.sub(0)); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Copy-constructor not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    
    EXPECT_EQ(24, nState1.data().size());
    for ( int i = 0; i < nState1.data().size(); ++i ) { EXPECT_EQ(0,nState1.data()(i)); }
}
TEST(TestStateMap, CopyAssignmentOperatorVector) {
    ...
    try                                   { nState1 = NestedVectorLvl1(nState0); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Copy-constructor not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    
    try                                   { nState1.sub(0) = NestedVectorLvl0(nState0.sub(0)); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Copy-constructor not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
...
TEST(TestStateMap, CopyAssignmentOperatorTuple) {
    ...
    for ( int i = 0; i < nState1.sub<0>().sub<0>().data().size(); ++i ) { nState1.sub<0>().sub<0>().data()(i) = 0; }
    for ( int i = 0; i < nState0.sub<0>().sub<0>().data().size(); ++i ) { EXPECT_EQ(i+1,nState0.sub<0>().sub<0>().data()(i)); }
    
    try                                   { nState1 = NestedTupleLvl2(nState0); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Copy-constructor not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    
    try                                   { nState1.sub<0>() = NestedTupleLvl1(nState0.sub<0>()); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Copy-constructor not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
...
TEST(TestStateMap, NestedTupleCompileTimeSize) {
    ...
    using NestedTupleDynInLvl2 = StateMapTuple<double, NestedTupleConst, NestedArray6Array6Vector, NestedArray6Array6Vector>;
    ...
    NestedTupleDynInLvl1 nDynLvl1; nDynLvl1.sub<2>().subResize(2);
    ...
    NestedArray6Array6Vector nDynArArVec; nDynArArVec.sub(0).sub(0).subResize(2);
    ...
    NestedTupleDynInLvl2 nDynLvl2; nDynLvl2.sub<1>().sub(0).sub(0).subResize(2);
...
TEST(TestStateMap, NestedStateResize) {
    ...
    for(int i = 0; i < nState.data().size(); ++i) { nState.data()(i) = 1; }
//     cout<<nState.data().transpose()<<endl;
    nState.sub<2>().subResize(2);
    EXPECT_EQ(4+2*4, nState.data().size());
//     cout<<nState.data().transpose()<<endl;
    for(int i = 0; i < nState.sub<2>().sub(0).data().size(); ++i) { nState.sub<2>().sub(0).data()(i) = 2; }
    for(int i = 0; i < nState.sub<2>().sub(1).data().size(); ++i) { nState.sub<2>().sub(1).data()(i) = 3; }
//     cout<<nState.data().transpose()<<endl;
    nState.sub<1>().sub(0).sub(0).subResize(1); for(int i = 0; i < nState.sub<1>().sub(0).sub(0).data().size(); ++i) { nState.sub<1>().sub(0).sub(0).data()(i) = 4; } 
    EXPECT_EQ(4+2*4+1, nState.data().size());
    nState.sub<1>().sub(0).sub(1).subResize(2); for(int i = 0; i < nState.sub<1>().sub(0).sub(1).data().size(); ++i) { nState.sub<1>().sub(0).sub(1).data()(i) = 5; }
    EXPECT_EQ(4+2*4+1+2, nState.data().size());
    nState.sub<1>().sub(1).sub(0).subResize(3); for(int i = 0; i < nState.sub<1>().sub(1).sub(0).data().size(); ++i) { nState.sub<1>().sub(1).sub(0).data()(i) = 6; }
    EXPECT_EQ(4+2*4+1+2+3, nState.data().size());
//     cout<<nState.data().transpose()<<endl;
    nState.sub<0>().sub<0>().subResize(3); for(int i = 0; i < nState.sub<0>().sub<0>().data().size(); ++i) { nState.sub<0>().sub<0>().data()(i) = 7; }
    EXPECT_EQ(4+2*4+1+2+3+3, nState.data().size());
    nState.sub<0>().sub<2>().subResize(2); for(int i = 0; i < nState.sub<0>().sub<2>().data().size(); ++i) { nState.sub<0>().sub<2>().data()(i) = 8; }
    EXPECT_EQ(4+2*4+1+2+3+3+2*6, nState.data().size());
//     cout<<nState.data().transpose()<<endl;
    std::array<double,4+2*4+1+2+3+3+2*6> numRes = { 7, 7, 7, 1, 1, 1, 1, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 4, 5, 5, 6, 6, 6, 2, 2, 2, 2, 3, 3, 3, 3 };
    for ( size_t i = 0; i < numRes.size(); ++i ) { EXPECT_EQ(numRes[i], nState.data()(i)); }
    
    nState.sub<0>().sub<0>().subResize(1);
    nState.sub<2>().subResize(1);
    std::array<double,4+1*4+1+2+3+1+2*6> numRes2 = { 7, 1, 1, 1, 1, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 4, 5, 5, 6, 6, 6, 2, 2, 2, 2 };
    for ( size_t i = 0; i < numRes2.size(); ++i ) { EXPECT_EQ(numRes2[i], nState.data()(i)); }
    
    nState.sub<0>().sub<0>().subResize(0);
    nState.sub<2>().subResize(0);
    std::array<double,4+0*4+1+2+3+0+2*6> numRes3 = { 1, 1, 1, 1, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 4, 5, 5, 6, 6, 6 };
    for ( size_t i = 0; i < numRes3.size(); ++i ) { EXPECT_EQ(numRes3[i], nState.data()(i)); }
}
...
//     cout<<nState.data().transpose()<<endl;
    ...
    for(int i = 0; i < nState.data().size(); ++i) { EXPECT_EQ(numRes0[i], nState.data()(i) ); }
...
TEST(TestStateMap, NestedArraysDataBuffer) {
    ...
    for(int i = 0; i < nArrLvl1.data().size(); ++i) { EXPECT_EQ(9 - i % 3, nArrLvl1.data()(i)); }
    nArrLvl1.data().block(7,0,4,1) = inc4;
    for(int i = 7; i < 7+4; ++i) { EXPECT_EQ(i-7+1, nArrLvl1.data()(i)); }
    nArrLvl1.sub(0).data() = inc4;
    nArrLvl1.sub(2).data() = inc4;
    for(int i = 0; i < nArrLvl1.sub(0).data().rows(); ++i) { EXPECT_EQ(i+1, nArrLvl1.data()(i)); }
    for(int i = 2*(int)sizeLvl0; i < 2*(int)sizeLvl0+nArrLvl1.sub(3).data().rows(); ++i) { EXPECT_EQ(i+1-2*sizeLvl0, nArrLvl1.data()(i)); }
}
...
TEST(TestStateMap, VirtualBase) {
    ...
    for(int i = 0; i < nStateBaseVirt->data().size(); ++i) { nStateBaseVirt->data()(i) = i; }
//     cout<<nStateBaseVirt->data().transpose()<<endl;
    for(int i = 0; i < nStateBaseVirt->data().size(); ++i) { EXPECT_EQ(i, nStateBaseVirt->data()(i)); }
    
    try                                   { nStateBaseVirt->subResize(2); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Cannot resize an array"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    
    try                                   { nStateBaseVirt->sub(0).sub(0).sub(0); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Access of numeric Leaf using virtual \"sub\" function not allowed"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    ...
    for(int i = 0; i < nStateTupBaseVirt->data().size(); ++i) { nStateTupBaseVirt->data()(i) = i; }
    try                                   { nStateTupBaseVirt->subResize(2); FAIL() << "Expected std::runtime_error"; } 
    catch(std::runtime_error const & err) { EXPECT_EQ(std::string("Cannot resize a tuple"), err.what()); } 
    catch(...)                            { FAIL() << "Expected std::runtime_error"; }
    
    std::shared_ptr<NestedTupleArray4VectorArray6>                   nStateTupExt      = dynamic_pointer_cast<NestedTupleArray4VectorArray6>(nStateTupBaseVirt);
    std::shared_ptr<StateMapBaseCRTP<NestedTupleArray4VectorArray6>> nStateTupBaseCRTP = nStateTupExt;
    
    for(int i = 0; i < nStateTupBaseVirt->sub(1)  .data().size(); ++i) { EXPECT_EQ(i+6, nStateTupBaseVirt->sub(1)  .data()(i)); }
    for(int i = 0; i < nStateTupExt     ->sub<1>().data().size(); ++i) { EXPECT_EQ(i+6, nStateTupExt     ->sub<1>().data()(i)); }
    for(int i = 0; i < nStateTupBaseCRTP->sub<1>().data().size(); ++i) { EXPECT_EQ(i+6, nStateTupBaseCRTP->sub<1>().data()(i)); }
}
