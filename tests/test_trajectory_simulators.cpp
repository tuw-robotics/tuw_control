...
// 	stAcc   = [this](const size_t& _i){ return latticePtr_->at(_i).statePtr->state( asInt(StateSimDiffDrive::StateVars::V) ); }; 
...
//     public  : size_t latFuncLayerIdx() override { return 2; }
// };
// class DiffDriveConstVOnKnots      : public cost_functions::CostsArrayOnLatKnotVec     <TrajectorySimulator::LatticeVec, double, DiffDriveCostConstrV   > {
...
//     public  : size_t latFuncLayerIdx() override { return 2; }
// };
// class DiffDriveConstVBetweenKnots : public cost_functions::CostsArrayBetweenLatKnotVec<TrajectorySimulator::LatticeVec, double, DiffDriveCostConstrVMin> {
...
//     
//     public   : DiffDriveTrajectoryOptimizer ( StateSimPtr& _stateSim, std::unique_ptr<CostsEvaluatorClass> _costsEvaluator ) : TrajectoryOptimizer(_stateSim, std::move(_costsEvaluator) ) {}
...
protected:
	...
	pf[asInt(PFV::W)].evalReq[asInt(PFFeM::INT1)] = false; pf[asInt(PFV::W)].evalReq[asInt(PFFeM::INT2)] = false;    
...
//             for ( size_t j = 0; j < _lattice[i].statePtr->stateSize(); ++j ) { cout<<_lattice[i].statePtr->value(j)<<", "; }cout<<endl;
//         }cout<<endl;
        for ( size_t i = 1; i < trajSim->simulationLattice_.size(); ++i ) { if( !(trajSim->simulationLattice_[i-1].arc <= trajSim->simulationLattice_[i].arc) ) { consistent = false; break; } }
        return consistent;
    }
    ...
    void checkSimLatticeStatesEqual ( vector <TrajectorySimulator::LatticePointType >& _latticeRef, vector <TrajectorySimulator::LatticePointType >& _latticeTest ) {
//         for ( size_t i = 0; i < _latticeTest.size(); ++i ) { 
//             for ( size_t j = 0; j < _latticeTest[i].statePtr->stateSize(); ++j ) { cout<<_latticeRef[i].statePtr->value(j)<<":"<<_latticeTest[i].statePtr->value(j)<<", "; }cout<<endl;
//         }cout<<endl;
        ASSERT_EQ( _latticeRef.size(), _latticeTest.size() ); 
        for ( size_t i = 0; i < _latticeTest.size(); ++i ) { 
            for ( size_t j = 0; j < _latticeTest[i].statePtr->valueSize(); ++j ) { EXPECT_DOUBLE_EQ( _latticeRef[i].statePtr->value(j), _latticeTest[i].statePtr->value(j) ); }
        }
    }
    void checkPartLatticeStatesEqual ( TrajectorySimulator::LatticeVecSPtrVec& _partLatRef, TrajectorySimulator::LatticeVecSPtrVec& _partLatTest ) {
...
TEST_F ( TrajectorySimulatorsTest, SimPrecalcFromNon0DtDsCtrlPt ) {
    ...
    for(size_t i = 0; i < trajSim->simulationLattice_.size(); ++i){ 
        ...
        trajStatesRef.emplace_back( TrajectorySimulator::LatticePointType( trajSim->simulationLattice_[i].arc, trajSim->simulationLattice_[i].latticeType, stateNow ) ); 
    }
    TrajectorySimulator::LatticeVecSPtrVec partLatRef; partLatRef.resize(trajSim->partLattices_.size() ); 
    for(auto& latRefI : partLatRef) { latRefI = make_shared<TrajectorySimulator::LatticeVec>();}
	...
	for(size_t j = 0; j < trajSim->partLattices_[i]->size(); ++j){
	    partLatRef[i]->at(j).arc = trajSim->partLattices_[i]->at(j).arc; partLatRef[i]->at(j).statePtr = trajSim->partLattices_[i]->at(j).statePtr->cloneState();
	}
    }
    
    trajSim->simulateTrajectory(0.10); checkSimLatticeStatesEqual( trajStatesRef, trajSim->simulationLattice_ ); checkPartLatticeStatesEqual( partLatRef, trajSim->partLattices_ );
    trajSim->simulateTrajectory(0.34); checkSimLatticeStatesEqual( trajStatesRef, trajSim->simulationLattice_ ); checkPartLatticeStatesEqual( partLatRef, trajSim->partLattices_ );
    trajSim->simulateTrajectory(0.79); checkSimLatticeStatesEqual( trajStatesRef, trajSim->simulationLattice_ ); checkPartLatticeStatesEqual( partLatRef, trajSim->partLattices_ );
    trajSim->simulateTrajectory(1.00); checkSimLatticeStatesEqual( trajStatesRef, trajSim->simulationLattice_ ); checkPartLatticeStatesEqual( partLatRef, trajSim->partLattices_ );
    trajSim->simulateTrajectory(0.54); checkSimLatticeStatesEqual( trajStatesRef, trajSim->simulationLattice_ ); checkPartLatticeStatesEqual( partLatRef, trajSim->partLattices_ );
    trajSim->simulateTrajectory(1.50); checkSimLatticeStatesEqual( trajStatesRef, trajSim->simulationLattice_ ); checkPartLatticeStatesEqual( partLatRef, trajSim->partLattices_ );
}
